...
import '../../widgets/admin/admin_propertytype_card.dart';

class AdminPropertyTypeManagement extends StatefulWidget {
  final bool canEdit;
  final bool canDelete;
  final bool canCreate;

  AdminPropertyTypeManagement({
    Key? key,
    this.canEdit = false,
...
class _AdminPropertyTypeManagementState
    extends State<AdminPropertyTypeManagement> {
  @override
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      body: Obx(
        () => Wrap(
          children: lookup.dealTypes
              .map((e) => AdminPropertyTypeCard(
                    type: e,
                    onEdit: !widget.canEdit
                        ? null
                        : () async {
                            // print("AdminPropertyTypeCard");
                            // Get.dialog(Center(
                            //   child: Card(
                            //     child: Column(
                            ...
                            //                 "/admin/propertytype/${e.id}",
                            //                 body: jsonEncode(body));

                            //             if (res.statusCode == HttpStatus.ok) {
                            //               Navigator.of(context).pop();

                            //               await lookup.loadTypes();
      ...
      floatingActionButton: !widget.canCreate
          ...
          : FloatingActionButton(
              ...
              onPressed: () async {
                ...
                //             var res = await Http().post("/admin/propertytype",
                //                 body: jsonEncode(data));

                //             if (res.statusCode == HttpStatus.created) {
                //               lookup.addType(PropertyTypeModel.fromJson(
                //                   utf8.decode(res.bodyBytes)));
